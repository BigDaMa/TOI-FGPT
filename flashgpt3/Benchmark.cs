...
namespace FlashGPT3
{
    public class Names
    {
        public static string resfilename = "../../../../results/res_toi_gpt4_cheat.txt";
    }
    public class BenchmarkRunner
    {
        public static void Main()
        {
            ...
            string[] semantic = BenchmarkUtils.GetBenchmarks("../../../../benchmarks/runningds/");
            //string[] syntactic = BenchmarkUtils.GetBenchmarks("../../../../benchmarks/syntactic/", extension: "json");
            //string[] semantic = new string[] { "../../../../benchmarks/semantic/teaching_adverb.csv" };
            ...
            // gpt3 without clustering
            //WitnessFunctions.Clustering = false;
            //Run(semantic, new BenchmarkSettings(grammarName: "GPT3",
            //                                    temperature: 0.0,
            //                                    query: "short",
            //                                    outputDir: Path.Combine(BenchmarkUtils.DefaultOutputDir,
            //                                                            "artefacts_gpt3")));

            // flashgpt3 without clustering
            //WitnessFunctions.Clustering = false;
            //Run(semantic, new BenchmarkSettings(grammarName: "FlashGPT3",
            //                                    temperature: 0.0,
            //                                    query: "short",
            //                                    outputDir: Path.Combine(BenchmarkUtils.DefaultOutputDir,
            //                                                            "artefacts_flashgpt3_nocluster")));

            //WitnessFunctions.Clustering = false;
            //Run(syntactic, new BenchmarkSettings(
            //    grammarName: "GPT3",
            //    temperature: 0.0,
            //    query: "short",
            //    outputDir: Path.Combine(BenchmarkUtils.DefaultOutputDir,
            ...
            //RunFlashFill(
            //    syntactic,
            //    new SyntacticBenchmarkSettings(Path.Combine(BenchmarkUtils.DefaultOutputDir,
            //                                   "artefacts_syntactic"))
            //);
        ...
        public static void Run(string[] files, BenchmarkSettings settings)
        {
            ...
            if (!Directory.GetParent(Names.resfilename).Exists)
                Directory.GetParent(Names.resfilename).Create();
            else
            {
                using (StreamWriter ofile = new StreamWriter(Names.resfilename))
                {
                    ofile.Write("");
                }
            }
            foreach (string inputFile in files)
            {
                ...
                using (StreamWriter ofile = File.AppendText(Names.resfilename))
                {
                    ofile.WriteLine($">>> {Path.GetFileNameWithoutExtension(inputFile)}");
                    ofile.Write("[");
                    ofile.Close();
                }
    ...
    public class BenchmarkSettings
    {
        ...
        public Query Builder()
        {
            ...
            if (query == "llama")
                return new LLaMAShortQuery();
            if (query == "mistral")
                return new MistralShortQuery();
    ...
    public class Benchmark
    {
        ...
        public BenchmarkResult Run()
        {
            ...
            for (int i = 1; i < Math.Min(settings.maxExamples + 1, problem.Count); i++)
            {
                ...
                stopwatch.Restart();
                // true = nocheat
                SaveTOICache(spec, TOICache, false);
                var task = Task.Run(() => 
                ...
                if (task.Wait(BenchmarkUtils.timeout))
                {
                    ...
                    using (StreamWriter outputFile = File.AppendText(Names.resfilename))
                    {
                        outputFile.Write(result.Accuracy(i).ToString());
                        if(i < settings.maxExamples) outputFile.Write(", ");
                        outputFile.Close();
                    }
